...
- useState 함수는 2개 짜리 배열을 반환하는 함수로, 첫 번째 값을 value로, 두 번째 값을 setter로 사용 가능합니다. useState가 객체가 아닌 배열을 반환하는 이유는 객체 구조 분해 할당은 사용하는 쪽에서 원하는 이름으로 변경하는 것이 번거롭기 때문에 자유롭게 이름을 선언하고자 배열을 반환하는 것으로 추측할 수 있습니다.
...
- 배열의 구조 분해 할당은 ,의 위치에 따라 값이 결정되는데, 실수를 유발할 가능성이 커서 일반적으로 배열의 길이가 작을 때 주로 쓰입니다. 값이 없는 경우(undefined) 기본값을 사용할 수 있습니다. 반드시 undefined일 때만 기본값을 사용하기 때문에 아래 구조 분해 할당에서 기본값을 사용하는 것은 a, e입니다.
